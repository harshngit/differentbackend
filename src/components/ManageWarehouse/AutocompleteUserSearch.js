...
const AutocompleteUserSearch = ({ selected, setSelected, data, placeholder, filterBySearchClient }) => {
	...
	return (
		<div className="">
			<Combobox value={selected} onChange={(person) => {
				setSelected(person); // ✅ Set the selected person
				setQuery(person.name); // ✅ Update query with selected name
				filterBySearchClient({ target: { value: person.name } }); // ✅ Trigger search
			}}>
