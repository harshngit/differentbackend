...
const ProductForm = ({
	...
	return (
		<div className="bg-[#fff] my-6 mx-6 rounded-lg py-5 px-5 h-[90vh] overflow-y-scroll">
			<h2 className="text-secondary font-bold text-[1.5rem]">Create Product</h2>
			<div className="flex flex-col gap-5 mb-5 mt-5">
				<div className="">
					<Radio defaultChecked={true}
						onChange={(e) => setProductSkuDetails(e.target.value)}
						value="manual" name='type' label="Manual" />
					<Radio
						onChange={(e) => {
							setProductSkuDetails(e.target.value)
						}}
						value="automatic" name='type' label="Automatic" />

					<div className='w-[400px] my-2' >
						<Input
							value={productSku} onChange={(e) => setProductSku(e.target.value)} disabled={productSkuDetails === "automatic" ? true : false}
							label='Enter SKU' />
					</div>
				</div>
			</div>
			...
			<div className="flex justify-end mt-5">
				<Button onClick={handleProduct} disabled={loading}>
					{loading ? "Submitting" : "Create Product"}</Button>
			</div>
		</div>
	);
};
