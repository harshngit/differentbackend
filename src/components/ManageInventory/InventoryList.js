...
import TopFilter from '../Products/TopFilter';
import TopInventoryNav from './TopInventoryNav';


const InventoryList = ({ productList, handleStatusChange, handleInventoryModal, filterBySearch, activeTab, setActiveTab }) => {
	return (
		<div className=''>
			<div className='flex justify-start px-5 py-5 items-end'>
				<h2 className='w-[50%] font-[GilroyBold] text-[25px]'>Inventory List</h2>
			...
			<div>
				<TopInventoryNav setActiveTab={setActiveTab} activeTab={activeTab} />
			</div>
			<div className={`bg-gray-300  py-4 px-8 grid  gap-6 grid-flow-row  items-center justify-start grid-cols-2 lg:grid-cols-9  `}>
				...
				<p className='col-span-1 font-[GilroyMedium]' >Inventory Status</p>
				...
				{productList?.map((item) => (
					<div>
						<div className={`bg-white  py-4 px-8 grid  gap-6 grid-flow-row  items-center justify-start grid-cols-2 lg:grid-cols-9  `}>
							...
							<p className='col-span-1 font-[GilroyMedium]' >{item?.productSize} </p>
		...
		${item?.productInventoryStatus === "Committed" ? "bg-green-100 text-green-600 border-green-600" :
										item?.productInventoryStatus === "Unavailable" ? "bg-red-100 text-red-600 border-red-600" :
											item?.productInventoryStatus === "Available" ? "bg-gray-200 text-black border-black" :
								...
								<Menu className=" rounded-none px-0 py-0">
									...
									<MenuList className="w-[150px] px-0 rounded-none">
										...
										<MenuItem className="border-b-[1px] rounded-none px-0 border-gray-300 "
											onClick={() => handleInventoryModal(item.id)}
										>
